...
```{r}
# Duration en minuts
for(i in top250movies$Duration){
  value <- str_split(string = i,
                     pattern = ' ')[[1]]
  if(is.na(value[2])){
    if(grepl(pattern = 'h',
             x = value[1])){
      hours <- str_remove_all(string = value[1],
                              pattern = '[a-z]')
      minutes <- 0
      }else{
        hours <- 0
        minutes <- str_remove_all(string = value[1],
                                  pattern = '[a-z]')
    }
    }else{
      hours <- str_remove_all(string = value[1],
                              pattern = '[a-z]')
      minutes <- str_remove_all(string = value[2],
                                pattern = '[a-z]')
      }
  top250movies$Duration[top250movies$Duration == i] <- as.numeric(hours)*60 + as.numeric(minutes)
}

# Review en unitats
for(i in top250movies$Review){
  if(grepl(pattern = 'K',
           x = i)){
    result <- paste(str_remove_all(string = i,
                                   pattern = '[^0-9]'),
                    '000',
                    sep = '')}else{
                      if(grepl(pattern = '\\.',
  ...
  top250movies$Review[top250movies$Review == i] <- result
}
...
top250movies.dollar_conversion <- function(local){
  return(Monetary_conversion_policy$Dolar[Monetary_conversion_policy$Local == local])}

for(i in Monetary_conversion_policy$Local)
for(j in top250movies$Budget[grepl(pattern = paste('\\b',
                                                   i,
                                                   sep = ''),
                                   x = top250movies$Budget)]){
    top250movies$Budget[top250movies$Budget == j] <- as.numeric(str_remove_all(string = j,
                                                                               pattern = '[^0-9]'))*top250movies.dollar_conversion(local = i)
  }
```
...
## Naturalesa de les característqiues
...
for(i in colnames(top250movies)){
  if((i %in% c('Classification', 'Director'))){
