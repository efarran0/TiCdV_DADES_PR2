...
# Detecció de valors
  top250movies.detect <- function(column,
    ...
    ifelse(test = is.na(input),
           yes = result <- NA,
           no = result <- grepl(pattern = pattern,
                                x = input))
    return(result)
  }
...
for(column in colnames(top250movies)){
  ...
  # Variables de tipus integer
    if(column %in% c('Year', 'Duration', 'Review', 'Budget', 'Collection')){
      top250movies[[column]] <- as.integer(top250movies[[column]])
    }
  
  # Variables de tipus factor
    if(column %in% c('Classification', 'Director')){
...
# Genre
  ...
  for(genre in genre_list){
    top250movies[[paste('Is_',
                        genre,
                        sep = '')]] <- top250movies.detect(column = 'Genre',
                                                           pattern = genre)
  }
```
...
```{r}
for(column in colnames(top250movies)){
  if(column %in% c('Budget', 'Collection')){
...
# Es comprova si la pel·lícula és rendible només si Collection és superior a 2 vegades Budget
  top250movies$Profitable <- top250movies.is_profitable(minimum_criteria = 2)
```
...
```{r}
for(column in colnames(top250movies)){
  if(class(top250movies[[column]]) %in% c('numeric', 'integer') &
     column != 'Year'){
    top250movies[[column]] <- top250movies.minmax_norm(column = column)
  }
}
```
