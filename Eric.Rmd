...
En aquest projecte analític, essencialment, es vol estudiar quina relació guarda la rendibilitat de les pel·lícules amb les altres característiques que es recullen en el dataset generat en la pràctica 1. La condició de renibilitat pels metratges s'esbrinarà més endavant, en l'apartat de generació de noves característiques, considerant només el pressupost, els ingressos totals i un llindar, encara per definir.
...
# Visualització
  top250movies.plot <- function(column){
    if ((class(top250movies[[column]]) != 'factor') &
        (str_detect(string = column,
                    pattern = 'norm',
                    negate = TRUE))) {
      p <-
        ggplot(data = NULL,
               aes(y = top250movies[[column]])) +
...
# Importació i descripció del dataset
...
# Es visualitzen els primers 3 registres de top250movies
  head(top250movies,
       n = 3)
  
# S'observa la dada per a cada atribut de top250movies
...
S'ometen el títol, la classificació i  el director de la pel·lícula per considerar-se dades irrellevants en aquest estudi. Tot i no emprar-se directament l'any, la característica serà d'utilitat posteriorment per a imputar valors perduts en el conjunt de dades.
  ...
  for (column in colnames(top250movies)) {
    if ((class(top250movies[[column]]) %in% c('numeric',
      ...
      top250movies[[paste(column,
                          '_z-score',
                          sep = '')]] <- top250movies.scale(column = column)
    }
  }
```
...
Posteriorment a la neteja del conjunt de dades es procedeix a visualitzar les distribucions de les característiques

```{r visualization}
# Visualització
  columns <- colnames(top250movies)
  criteria <- str_detect(string = colnames(top250movies),
                                           pattern = 'norm',
                                           negate = TRUE)
  
  gg <- lapply(X = columns[criteria],
               FUN = top250movies.plot)
```

```{r display 1, echo = FALSE, fig.cap = 'Distrubició variables numèriques', fig.align = 'center'}
do.call(what = 'grid.arrange', args = c(gg[1:6],
                                        gg[28],
                                        ncol = 2))
```

```{r display 2, echo = FALSE, fig.cap = 'Distrubició variables categòriques', fig.align = 'center'}
do.call(what = 'grid.arrange', args = c(gg[7:14],
                                        ncol = 2))
```

```{r display 3, echo = FALSE, fig.cap = 'Distrubició variables categòriques', fig.align = 'center'}
do.call(what = 'grid.arrange', args = c(gg[15:22],
                                        ncol = 2))
```

```{r display 4, echo = FALSE, fig.cap = 'Distrubició variables categòriques', fig.align = 'center'}
do.call(what = 'grid.arrange', args = c(gg[23:27],
                                        gg[29],
                                        ncol = 2))
```
